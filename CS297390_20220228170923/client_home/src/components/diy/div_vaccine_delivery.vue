<template>
	<div class="diy_details_box diy_div_vaccine_delivery">
		<div class="warp">
			<div class="container">
	...
	export default {
		...
		methods: {
			...
			change_praise(obj) {
				...
				var query = {
					source_table: "vaccine_delivery",
					source_field: "vaccine_delivery_id",
					source_id: this.obj["vaccine_delivery_id"],
					user_id
				};
				...
				if (this.state_praise) {
					...
					this.$get('~/api/praise/del?', query, (res) => {
						if(res.result){
							...
							this.$post('~/api/vaccine_delivery/set?vaccine_delivery_id=' + _this.obj["vaccine_delivery_id"], {
								praise_len
							}, (res) => {
								if(res.result){
