<template>
	<div class="diy_details_box diy_div_vaccine_information">
		<div class="warp">
			<div class="container">
				...
				<div class="row">
						...
						<!-- 图片 -->
							...
							<div class="details_btn_wrap">
								<button class="btn btn-primary details_btn" v-if="$check_action('/reservation_plan/edit','add')" @click="to_form('/reservation_plan/edit')" >添加计划 </button>
								<button class="btn btn-primary details_btn" v-if="$check_action('/warehoused_vaccine/edit','add')" @click="to_form('/warehoused_vaccine/edit')" >入库 </button>
								<button class="btn btn-primary details_btn" v-if="$check_action('/vaccine_delivery/edit','add')" @click="to_form('/vaccine_delivery/edit')" >出库 </button>
							</div>
						<!-- 富文本 -->
	...
	export default {
		...
		methods: {
			...
			change_praise(obj) {
				...
				var query = {
					source_table: "vaccine_information",
					source_field: "vaccine_information_id",
					source_id: this.obj["vaccine_information_id"],
					user_id
				};
				...
				if (this.state_praise) {
					...
					this.$get('~/api/praise/del?', query, (res) => {
						if(res.result){
							...
							this.$post('~/api/vaccine_information/set?vaccine_information_id=' + _this.obj["vaccine_information_id"], {
								praise_len
							}, (res) => {
								if(res.result){
