<template>
	<div class="diy_details_box diy_div_appointment_vaccine">
		<div class="warp">
			<div class="container">
	...
	export default {
		...
		methods: {
			...
			change_praise(obj) {
				...
				var query = {
					source_table: "appointment_vaccine",
					source_field: "appointment_vaccine_id",
					source_id: this.obj["appointment_vaccine_id"],
					user_id
				};
				...
				if (this.state_praise) {
					...
					this.$get('~/api/praise/del?', query, (res) => {
						if(res.result){
							...
							this.$post('~/api/appointment_vaccine/set?appointment_vaccine_id=' + _this.obj["appointment_vaccine_id"], {
								praise_len
							}, (res) => {
								if(res.result){
