<template>
	<div class="diy_details_box diy_div_warehoused_vaccine">
		<div class="warp">
			<div class="container">
	...
	export default {
		...
		methods: {
			...
			change_praise(obj) {
				...
				var query = {
					source_table: "warehoused_vaccine",
					source_field: "warehoused_vaccine_id",
					source_id: this.obj["warehoused_vaccine_id"],
					user_id
				};
				...
				if (this.state_praise) {
					...
					this.$get('~/api/praise/del?', query, (res) => {
						if(res.result){
							...
							this.$post('~/api/warehoused_vaccine/set?warehoused_vaccine_id=' + _this.obj["warehoused_vaccine_id"], {
								praise_len
							}, (res) => {
								if(res.result){
