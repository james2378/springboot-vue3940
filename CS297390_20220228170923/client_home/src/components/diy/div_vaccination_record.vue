<template>
	<div class="diy_details_box diy_div_vaccination_record">
		<div class="warp">
			<div class="container">
	...
	export default {
		...
		methods: {
			...
			change_praise(obj) {
				...
				var query = {
					source_table: "vaccination_record",
					source_field: "vaccination_record_id",
					source_id: this.obj["vaccination_record_id"],
					user_id
				};
				...
				if (this.state_praise) {
					...
					this.$get('~/api/praise/del?', query, (res) => {
						if(res.result){
							...
							this.$post('~/api/vaccination_record/set?vaccination_record_id=' + _this.obj["vaccination_record_id"], {
								praise_len
							}, (res) => {
								if(res.result){
