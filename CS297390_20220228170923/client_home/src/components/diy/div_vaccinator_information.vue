<template>
	<div class="diy_details_box diy_div_vaccinator_information">
		<div class="warp">
			<div class="container">
	...
	export default {
		...
		methods: {
			...
			change_praise(obj) {
				...
				var query = {
					source_table: "vaccinator_information",
					source_field: "vaccinator_information_id",
					source_id: this.obj["vaccinator_information_id"],
					user_id
				};
				...
				if (this.state_praise) {
					...
					this.$get('~/api/praise/del?', query, (res) => {
						if(res.result){
							...
							this.$post('~/api/vaccinator_information/set?vaccinator_information_id=' + _this.obj["vaccinator_information_id"], {
								praise_len
							}, (res) => {
								if(res.result){
