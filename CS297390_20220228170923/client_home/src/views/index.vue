		        ...
		        <div class="row">
		            <div class="col-12">
		                ...
		                <div class="diy_recommend">
		                    ...
		                    <div class="more_box">
		                        <router-link to="/vaccine_information/list" class="more">
		                            <span>更多</span>
		                        </router-link>
		                    </div>
		                    <div class="switch" @click="show_list_vaccine_information = !show_list_vaccine_information">
		                        <div class="switch_box">
		                            <span v-if="show_list_vaccine_information"> 切换表格 </span>
		                            <span v-else> 切换列表 </span>
		                        </div>
		                    </div>
		                </div>
		                <!--疫苗信息推荐列表组件 -->
		                <list_vaccine_information v-if="show_list_vaccine_information" :list="list_vaccine_information" />
		                <div class="overflow-auto" v-else>
		                    <table id="list_diy" role="table" aria-busy="false"
		                           :aria-colcount="fields_vaccine_information.length"
		                           class="table b-table table-striped table-hover">
		                        <thead>
		                        <tr>
		                            <th v-for="(o,i) in fields_vaccine_information" :key="i">
		                                {{o.label}}
		                            </th>
		                        </tr>
		                        </thead>
		                        <tbody>
		                        <tr v-for="(o, i) in list_table_vaccine_information" :key="i" @click="to_details('vaccine_information',o,'vaccine_information_id')">
		                            <td v-for="(oj,n) in fields_vaccine_information">
		                                <img v-if="oj.type && oj.type == '图片' " :src="$fullUrl(o[oj.key])" alt=""
		                                     v-default-img="'/img/default.png'">
		        ...
		        <div class="row">
		            <div class="col-12">
		                ...
		                <div class="diy_recommend">
		                    ...
		                    <div class="more_box">
		                        <router-link to="/reservation_plan/list" class="more">
		                            <span>更多</span>
		                        </router-link>
		                    </div>
		                    <div class="switch" @click="show_list_reservation_plan = !show_list_reservation_plan">
		                        <div class="switch_box">
		                            <span v-if="show_list_reservation_plan"> 切换表格 </span>
		                            <span v-else> 切换列表 </span>
		                        </div>
		                    </div>
		                </div>
		                <!--预约计划推荐列表组件 -->
		                <list_reservation_plan v-if="show_list_reservation_plan" :list="list_reservation_plan" />
		                <div class="overflow-auto" v-else>
		                    <table id="list_diy" role="table" aria-busy="false"
		                           :aria-colcount="fields_reservation_plan.length"
		                           class="table b-table table-striped table-hover">
		                        <thead>
		                        <tr>
		                            <th v-for="(o,i) in fields_reservation_plan" :key="i">
		                                {{o.label}}
		                            </th>
		                        </tr>
		                        </thead>
		                        <tbody>
		                        <tr v-for="(o, i) in list_table_reservation_plan" :key="i" @click="to_details('reservation_plan',o,'reservation_plan_id')">
		                            <td v-for="(oj,n) in fields_reservation_plan">
		                                <img v-if="oj.type && oj.type == '图片' " :src="$fullUrl(o[oj.key])" alt=""
		                                     v-default-img="'/img/default.png'">
										<span v-else-if="oj.type && oj.type == 'UID' ">
											<span style="display: none">
	...
	export default {
		...
		data() {
			return {
				...
				show_list_vaccine_information: true,
				list_vaccine_information: [],
				vm_arr_vaccine_information: [
				...
				fields_vaccine_information: [
					...
					{
						key: "suitable_for_people",
						label: "适合人群",
						type: "文本"
					},
					{
						key: "vaccine_producing_area",
						label: "疫苗出产地",
						type: "下拉"
					},
					{
				...
				show_list_reservation_plan: true,
				list_reservation_plan: [],
				vm_arr_reservation_plan: [
					"reservation_plan_id",
					"vaccine_number",
					"vaccine_type",
					"vaccine_name",
		...
		methods: {
			...
			get_vaccine_information() {
				let url = "~/api/vaccine_information/get_list?";
				this.$get(url, {
					"page": 1,
					"size": 4
				}, (json) => {
					if (json.result) {
						this.list_vaccine_information = json.result.list;
					}
				})
			},
			// 获取预约计划列表
			get_reservation_plan() {
				let url = "~/api/reservation_plan/get_list?";
				this.$get(url, {
					"page": 1,
					"size": 4
				}, (json) => {
					if (json.result) {
						this.list_reservation_plan = json.result.list;
					}
				})
			},
		...
		mounted() {
			this.get_vaccine_information();
			this.get_reservation_plan();
			this.get_menu();
			this.get_slides();
			this.get_article();
			this.get_link();
		...
		computed: {
		    list_table_vaccine_information() {
		        var list = this.list_vaccine_information;
		        var list_table = [];
		        for (let i = 0; i < list.length; i++) {
		            list_table[i] = {};
		            this.vm_arr_vaccine_information.map((o) => {
		                // 第二个中括号是对象的属性
		    ...
		    list_table_reservation_plan() {
		        var list = this.list_reservation_plan;
		        var list_table = [];
		        for (let i = 0; i < list.length; i++) {
		            list_table[i] = {};
		            this.vm_arr_reservation_plan.map((o) => {
		                // 第二个中括号是对象的属性
		                list_table[i][o] = list[i][o] || "";
		            });
		        }
